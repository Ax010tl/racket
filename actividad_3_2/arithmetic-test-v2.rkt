...
(define test-arithmetic-lexer
    (test-suite
        ...
        (check-equal? (arithmetic-lexer "5.23") '((float)) "Single float")
        (check-equal? (arithmetic-lexer "2+1") '(int operator int) "Binary operation ints")
        (check-equal? (arithmetic-lexer "5.2+3") '(float operator int) "Float and int")
        (check-equal? (arithmetic-lexer "5.2+3.7") '(float operator float) "Binary operation floats")
